...
class EVETrainer(Trainer):
        ...
        if self.optimizer is None:
            ...
            if self.args.vision_tower_lr is not None and self.args.vision_tower_lr != 0:
                ...
                optimizer_grouped_parameters = [
                    {
                        "params": [
                            p for n, p in opt_model.named_parameters() if (p.requires_grad and n in decay_parameters and n not in vision_tower_parameters)
                        ],
                        "weight_decay": self.args.weight_decay,
                    },
                    {
                        "params": [
                            p for n, p in opt_model.named_parameters() if (p.requires_grad and n in decay_parameters and n in vision_tower_parameters)
                        ],
                        "weight_decay": self.args.weight_decay,
                    ...
                    {
                        "params": [
                            p for n, p in opt_model.named_parameters() if (p.requires_grad and n not in decay_parameters and n not in vision_tower_parameters)
                        ],
                        "weight_decay": 0.0,
                    },
                    {
                        "params": [
                            p for n, p in opt_model.named_parameters() if (p.requires_grad and n not in decay_parameters and n in vision_tower_parameters)
                        ],
                        "weight_decay": 0.0,
                        "lr": self.args.vision_tower_lr,
                    },
                ...
                optimizer_grouped_parameters = [
                    {
                        "params": [
                            p for n, p in opt_model.named_parameters() if (p.requires_grad and n in decay_parameters)
                        ],
                        "weight_decay": self.args.weight_decay,
                    },
                    {
                        "params": [
                            p for n, p in opt_model.named_parameters() if (p.requires_grad and n not in decay_parameters)
                        ],
                        "weight_decay": 0.0,
                    },
                ]
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        super(EVETrainer, self)._save_checkpoint(model, trial, metrics)

    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        super(EVETrainer, self)._save(output_dir, state_dict)
